...
def forwardForced(mat, size, p, time, distances):
    """
...
def propagateForced(size, origin, p, maxt):
    """
    ...
    distances = np.zeros(size) # used for forced method only
    for i in range(size[0]):
        for j in range(size[1]):
            distances[i,j] = la.norm(np.array(origin)-[i,j])
    ...
    for time in range(maxt):
        if (fires[time,:,:] != 0).all() == True:
            ...
            break
        forwardForced(fires, size, p, time, distances)
    return fires


def forcedEnssemble(size, origin, p, maxt, maxr):
    """
    size: size of matrix
    ...
    for r in range(maxr): # run simulation maxr times
        fire = propagateForced(size, origin, p, maxt)
        avgFire[:,:,:] += fire
